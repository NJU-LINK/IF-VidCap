...
class VideoLLaMAInfer:
    def __init__(self,
                 model_name: str = "VideoLLaMA3-7B",
        ...
        self.model_path = os.path.join("./models/VideoLlama", model_name)
        ...
        # 根据文档，VideoLLaMA3-7B 支持最多 128 帧
        # 对于60秒视频，我们用 2fps 采样，刚好得到 120 帧
        self.fps = 2.0
        self.max_frames = 128

        print(f"视频处理配置: fps={self.fps}, max_frames={self.max_frames}")
            ...
            while retry_count < max_retries and not success:
                try:
                    ...
                    for i, prompt_info in enumerate(video_prompts):
                        ...
                        response = self.process_video(video_path, self.meta_prompt, prompt)
...
def main():
    parser = argparse.ArgumentParser(description="VideoLLaMA 视频理解推理脚本")
    parser.add_argument(
        "--model_name",
        type=str,
        default="VideoLLaMA3-7B",
        help="模型名称，如 VideoLLaMA3-7B、VideoLLaMA3-2B 等"
    )
    parser.add_argument(
        "-i", "--input_dir",
        type=str,
        default="./annotation/normal",
        help="输入目录，包含 prompt.json"
    )
    parser.add_argument(
        "-o", "--output_dir",
        type=str,
    ...
    parser.add_argument(
        "--thinking",
        action="store_true",
        help="启用思考模式，模型将首先进行思考然后给出回答"
    )

    args = parser.parse_args()

    # 初始化VideoLLaMA推理类
    infer = VideoLLaMAInfer(
...
if __name__ == "__main__":
    ...
    # 基本使用（VideoLLaMA3-7B）
    python get_response_VideoLlama.py --model_name VideoLLaMA3-7B
    
    # 使用不同的模型
    python get_response_VideoLlama.py --model_name VideoLLaMA3-2B
    python get_response_VideoLlama.py --model_name VideoLLaMA3-7B-Image

    # 启用思考模式
    python get_response_VideoLlama.py --model_name VideoLLaMA3-7B --thinking

    # 使用自定义输入输出目录
    python get_response_VideoLlama.py --model_name VideoLLaMA3-7B -i ./annotation/normal -o ./response_normal
    '''
