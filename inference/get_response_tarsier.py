...
from models.Tarsier.tools.utils import load_model_and_processor
from models.Tarsier.models.modeling_tarsier import TarsierForConditionalGeneration
...
class TestModel:
    def __init__(self,
        ...
        model_path = f"./models/Tarsier/{model}"
        config_path = f"./models/Tarsier/configs/data_config.yaml"
        ...
        if not hasattr(self.model.config, "attn_implementation"):
    ...
    def process_video_with_tarsier(self, video_path: str, meta_prompt: str, prompt: str):
        ...
        # 将视频帧作为 image 序列加入 content
        for frame_tensor in pixel_values:
            # 转成 PIL 再传给 processor
            img = T.ToPILImage()(frame_tensor)
    ...
    def process_video(self, video_path: str, prompt: str):
        ...
        调用处理函数。
        """
        return self.process_video_with_tarsier(video_path, self.meta_prompt, prompt)
...
if __name__ == "__main__":
    ...
    '''
    python get_response_tarsier.py --model Tarsier2-7b
    python get_response_tarsier.py --model Tarsier-34b

    # 使用自定义输入输出目录
    python get_response_tarsier.py --model Tarsier2-7b -i ./annotation/normal -o ./response_normal
    '''
